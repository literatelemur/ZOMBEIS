...
void World::render(Graphics* graphics){
    ...
    graphics->draw_hollow_triangles_sphere_as_lines(sphere_points_2D_as_lines);


    // Drawing floor lines

    std::vector<std::vector<std::vector<std::vector<double>>>> floor_points_3D = graphics->find_floor_lines_on_globe(sphere_triangle_points_3D);

    std::vector<std::vector<std::vector<std::vector<double>>>> clipped_ordered_floor_points_3D = graphics->clip_sphere_lines(floor_points_3D);
